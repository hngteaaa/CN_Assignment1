...
class ServerWorker:
	...
	def describe(self):
    	# Take server port from str
		str1 = str(self.clientInfo['rtspSocket'][0])
		server_port = ''
		count = 0
		count1 = 0
		for i in range(len(str1)):
			if str1[i] == '(':
				count1 += 1
			if str1[i] == ',' and count1 == 1:
				count += 1
			if count == 1 and str1[i] != ',' and str1[i] != ' ':
				if str1[i] == ')':
					break
				server_port += str1[i]

		seq1 = "Client port: " + str(self.clientInfo['rtpPort']) + " RTP/AVP 26\n" + "Server port: " + server_port + "\na=control:streamid=" \
